import 'package:flutter/cupertino.dart';

import 'color_constant.dart';

// Style for title
var mTitleStyle = const TextStyle(
  fontWeight: FontWeight.w600,
  color: mTitleColor,
  fontSize: 16,
);

// Style for Discount Section
var mMoreDiscountStyle = const TextStyle(
  fontSize: 12,
  fontWeight: FontWeight.w700,
  color: mBlueColor,
);

// Style for Service Section
var mServiceTitleStyle = const TextStyle(
  fontWeight: FontWeight.w500,
  fontSize: 12,
  color: mTitleColor,
);
var mServiceSubtitleStyle = const TextStyle(
  fontWeight: FontWeight.w400,
  fontSize: 10,
  color: mSubtitleColor,
);

// Style for Popular Destination Section
var mPopularDestinationTitleStyle = const TextStyle(
  fontWeight: FontWeight.w700,
  fontSize: 16,
  color: mCardTitleColor,
);
var mPopularDestinationSubtitleStyle = const TextStyle(
  fontWeight: FontWeight.w500,
  fontSize: 10,
  color: mCardSubtitleColor,
);

// Style for Travlog Section
var mTravlogTitleStyle = const TextStyle(
  fontSize: 14,
  fontWeight: FontWeight.w900,
  color: mFillColor,
);
var mTravlogContentStyle = const TextStyle(
  fontSize: 10,
  fontWeight: FontWeight.w500,
  color: mTitleColor,
);
var mTravlogPlaceStyle = const TextStyle(
  fontSize: 10,
  fontWeight: FontWeight.w500,
  color: mBlueColor,
);
